		...
		for (int test_case = 1; test_case <= 10; test_case++) {
			...
			for (int r = 2; r < n; r++) {
				for (int i = 0; i < n - r; i++) {
					int j = i + r;
					for (int k = i; k < j; k++) {
						CP[0][i][j] += CP[2][i][k] * CP[0][k+1][j] + CP[0][i][k] * CP[2][k+1][j] + CP[1][i][k] * CP[2][k+1][j];
						CP[1][i][j] += CP[0][i][k] * CP[0][k+1][j] + CP[0][i][k] * CP[1][k+1][j] + CP[1][i][k] * CP[1][k+1][j];
						CP[2][i][j] += CP[1][i][k] * CP[0][k+1][j] + CP[2][i][k] * CP[1][k+1][j] + CP[2][i][k] * CP[2][k+1][j];
					}
				}
			}

			/////////////////////////////////////////////////////////////////////////////////////////////
			Answer[0] = CP[0][0][n-1];  // a 의 갯수
			Answer[1] = CP[1][0][n-1];  // b 의 갯수
			Answer[2] = CP[2][0][n-1];  // c 의 갯수
