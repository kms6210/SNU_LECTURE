...
class Solution2 {
	...
	private static int parentCheck(int x) {
		if (parent[x] == x) {
			return x;
		} else {
			parent[x] = parent[parent[x]];
			return parentCheck(parent[x]);
		}
	}

	private static void parentChange(int a, int b) {
		int pa = parentCheck(a);
		int pb = parentCheck(b);
		parent[pa] = pb;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		for (int test_case = 1; test_case <= 10; test_case++) {
			...
			Answer = 0;
			for (int i = 0; i < E; i++) {
				if (parentCheck(newU[i]) != parentCheck(newV[i])) {
					parentChange(newU[i], newV[i]);
					Answer += newW[i];
				}
			}
